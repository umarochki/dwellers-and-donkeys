...
export default class VisionRegion extends PIXI.Container {
  ...
  constructor(drawer, viewport, renderer) {
    super();
    ...
    this.polygons = new PIXI.Container();
  ...
  compute(position, segments) {
    ...
    for (var i = 0; i < segments.length; ++i) {
      var a1 = this.angle(segments[i][0], position);
      var a2 = this.angle(segments[i][1], position);
      var active = false;
    ...
    for (var i = 0; i < sorted.length;) {
      ...
      if (extend) {
        polygon.push(vertex);
        var cur = this.intersectLines(segments[heap[0]][0], segments[heap[0]][1], position, vertex);
        if (!this.equal(cur, vertex)) polygon.push(cur);
      } else if (shorten) {
        polygon.push(this.intersectLines(segments[old_segment][0], segments[old_segment][1], position, vertex));
        polygon.push(this.intersectLines(segments[heap[0]][0], segments[heap[0]][1], position, vertex));
      } 
    }
    return polygon;
  };
