...
export interface IViewportOptions {
     ...
     */
    forceHitArea?: PIXI.Rectangle | null;
...
export class Viewport extends Container
{
    ...
    private _forceHitArea?: PIXI.IHitArea | null;
    ...
    public toWorld(x: number, y: number): PIXI.Point;
    /** Change coordinates from screen to world */
    public toWorld(screenPoint: PIXI.Point): PIXI.Point;

    public toWorld(x: number | PIXI.Point, y?: number): PIXI.Point
    {
        if (arguments.length === 2)
        {
            return this.toLocal(new PIXI.Point(x as number, y));
        }

        return this.toLocal(x as PIXI.Point);
    }

    /** Change coordinates from world to screen */
    public toScreen(x: number, y: number): PIXI.Point;
    /** Change coordinates from world to screen */
    public toScreen(worldPoint: PIXI.Point): PIXI.Point;

    public toScreen(x: number | PIXI.Point, y?: number): PIXI.Point
    {
        if (arguments.length === 2)
        {
            return this.toGlobal(new PIXI.Point(x as number, y));
        }

        return this.toGlobal(x as PIXI.Point);
    }
    ...
    public moveCenter(center: PIXI.Point): Viewport;

    public moveCenter(...args: [number, number] | [PIXI.Point]): Viewport
    {
        let x: number;
        let y: number;
    ...
    /** move Viewport's top-left corner; also clamps and resets decelerate and bounce (as needed) */
    public moveCorner(center: PIXI.Point): Viewport;

    public moveCorner(...args: [number, number] | [PIXI.Point]): Viewport
    {
        let x;
        let y;
     ...
     */
    get forceHitArea(): PIXI.IHitArea | null | undefined
    {
        return this._forceHitArea;
    }
    set forceHitArea(value: PIXI.IHitArea | null | undefined)
    {
        if (value)
        {
     ...
     * @returns {Viewport} this
     */
    public follow(target: PIXI.DisplayObject, options?: IFollowOptions): Viewport
    {
