...
export class Pinch extends Plugin
{
    ...
    protected lastCenter?: PIXI.IPointData | null;
    ...
    public move(e: PIXI.InteractionEvent): boolean
    {
        ...
        if (pointers.length >= 2)
        {
            ...
            {
                first.last = { x, y, data: e.data } as PIXI.IPointData;
            }
            else if (second.id === e.data.pointerId)
            {
                second.last = { x, y, data: e.data } as PIXI.IPointData;
            }
            if (last)
            {
                let oldPoint: PIXI.IPointData | undefined;

                const point = {
                    x: (first.last as PIXI.IPointData).x +
                        ((second.last as PIXI.IPointData).x - (first.last as PIXI.IPointData).x) / 2,
                    y: (first.last as PIXI.IPointData).y +
                        ((second.last as PIXI.IPointData).y - (first.last as PIXI.IPointData).y) / 2,
                };
                ...
                let dist = Math.sqrt(Math.pow(
                    (second.last as PIXI.IPointData).x - (first.last as PIXI.IPointData).x, 2) +
                    Math.pow((second.last as PIXI.IPointData).y - (first.last as PIXI.IPointData).y, 2));
                ...
                {
                    const newPoint = this.parent.toGlobal(oldPoint as PIXI.IPointData);
